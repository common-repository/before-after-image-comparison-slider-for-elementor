...
if( ! class_exists( 'Wp_ebaic_Usage_Feedback') ) {
	
	class Wp_ebaic_Usage_Feedback {
		
		private $wpbot_version = '1.3.2';
		private $home_url = '';
		private $plugin_file = '';
		private $plugin_name = '';
		...
		public function admin_enqueue_scripts(){
			wp_register_style( 'qcopd-custom-admin-css', WB_EBAIC_URL . 'assets/css/admin-style.css');
		...
		public function send_data( $body ) {
			...
			foreach($body as $key=>$value){
			    ...
			    $title   = esc_html('Elementor Before After Slider - Free Plugin Deactivation Notice');
		...
		public function deactivate_this_plugin() {
			...
			if( false !== get_option( 'wbebaic_deactivation_reason_' . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'wbebaic_deactivation_reason_' . $this->plugin_name );
				delete_option('wbebaic_deactivation_reason_' . $this->plugin_name);
			}
			if( false !== get_option( 'wbebaic_deactivation_details_' . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'wbebaic_deactivation_details_' . $this->plugin_name );
				delete_option('wbebaic_deactivation_details_' . $this->plugin_name);
			}

			if( false !== get_option( 'wbebaic_deactivation_email_' . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'wbebaic_deactivation_email_' . $this->plugin_name );
				delete_option('wbebaic_deactivation_email_' . $this->plugin_name);
			}

			if( false !== get_option( 'wbebaic_deactivation_main_reason_' . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'wbebaic_deactivation_main_reason_' . $this->plugin_name );
				delete_option('wbebaic_deactivation_main_reason_' . $this->plugin_name);
			}
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="javascript:event.preventDefault();" id="wpb-goodbye-link-sld-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function goodbye_ajax() {
			...
			<script>
				jQuery(document).ready(function($){
					 $(document).on('change', '#wpb-goodbye-form-before-after-image-comparison-slider-for-elementor input[type=radio]', function() { 
					...
					$('#wpb-goodbye-link-sld-<?php echo esc_attr( $this->plugin_name ); ?>').on('click',function(){
						var url = document.getElementById('wpb-goodbye-link-sld-<?php echo esc_attr( $this->plugin_name ); ?>');
		...
		public function goodbye_form_callback() {
			...
			if( isset( $_POST['values'] ) ) {
				...
				update_option( 'wbebaic_deactivation_reason_' . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'wbebaic_deactivation_details_' . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_text_field( $_POST['email'] );
				update_option( 'wbebaic_deactivation_email_' . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'wbebaic_deactivation_main_reason_' . $this->plugin_name, $main_reason );
			}
